...
Sigma_Exit:
        ...
        */
        ...
        case 931: // Set chop conf
          {
            if (code_seen('S')) {
              uint8_t hysteresis_start = code_value_uint8();
        ...
        case 932:
          {
            ...
            if (code_seen('A')) {
              tmc2130_set_pwm_ampl(E_AXIS, code_value_uint8());
              do_i_init = true;
            }
            if (code_seen('R')) {
              tmc2130_set_pwm_grad(E_AXIS, code_value_uint8());
              do_i_init = true;
            }
            if (code_seen('T')) {
              tmc2130_set_pwm_auto(E_AXIS, code_value_uint8());
              do_i_init = true;
            }
            if (code_seen('F')) {
              tmc2130_set_pwm_freq(E_AXIS, code_value_uint8());
              do_i_init = true;
            }
        ...
        */
        ...
        case 361: // Toggle Interpolation
          {
            if (code_seen('X')) {
              tmc2130_intpol[X_AXIS] = code_value();
              printf_P(PSTR("X Axis step interpolation is %i"), tmc2130_intpol[X_AXIS]);
            }
            if (code_seen('Y')) {
              tmc2130_intpol[Y_AXIS] = code_value();
              printf_P(PSTR("Y Axis step interpolation is %i"), tmc2130_intpol[Y_AXIS]);
            }
            if (code_seen('Z')) {
              tmc2130_intpol[Z_AXIS] = code_value();
              printf_P(PSTR("Z Axis step interpolation is %i"), tmc2130_intpol[Z_AXIS]);
            }
            if (code_seen('E')) {
              tmc2130_intpol[E_AXIS] = code_value();
              printf_P(PSTR("E Axis step interpolation is %i"), tmc2130_intpol[E_AXIS]);
            }
